...
def _load_custom_questions() -> list[dict[str, Any]]:
    try:
        data = json.loads(CUSTOM_FILE.read_text("utf-8"))
...
def _write_custom(questions: list[dict[str, Any]]) -> None:
...
def _normalize_item(raw: Any, idx: int, used_ids: set[str]) -> dict[str, Any]:
...
def list_questions(unit: str = "", status: str = "all", q: str = "") -> list[dict[str, Any]]:
...
def _ensure_problem(conn: Any, item: dict[str, Any]) -> int:
    ...
    cid = conn.execute(
        "SELECT id FROM concepts WHERE name = ?", (item.get("concept", ""),)
    ).fetchone()
    concept_csv = f",{int(cid[0])}," if cid else ""
    stamp = now()
    cursor = conn.execute("""
        INSERT INTO problems(title, course, topic, content, my_attempt, error_type,
                             concept_ids, methods, mastery, ease_factor, repetition, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 2.5, 0, ?, ?)
    """, (
        title,
        str(item.get("unit", "")),
    ...
    conn.execute(
        "INSERT OR REPLACE INTO bank_problems(qid, problem_id, updated_at) VALUES (?, ?, ?)",
        (item["id"], pid, stamp),
    )
    return pid


def judge(qid: str, answer: Any) -> dict[str, Any]:
